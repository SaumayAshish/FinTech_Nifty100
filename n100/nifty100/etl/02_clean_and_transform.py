...
def parse_year_label(value: object) -> dict[str, object]:
    ...
    if not raw:
        return {"year_label": None, "fiscal_year": None, "quarter": None, "is_ttm": False, "is_half_year": False, "sort_order": None}
    if raw.upper() == "TTM":
        return {"year_label": "TTM", "fiscal_year": None, "quarter": "TTM", "is_ttm": True, "is_half_year": False, "sort_order": 999999}

    decimal_match = re.fullmatch(r"(\d{4})\.5", raw)
    if decimal_match:
        year = int(decimal_match.group(1))
        return {"year_label": f"Sep {year}", "fiscal_year": year, "quarter": "Q2", "is_ttm": False, "is_half_year": False, "sort_order": year * 10 + 2}
    ...
    if match:
        ...
        is_half = mon in {"Sep", "Mar"} and raw.lower().startswith(("sep", "mar")) and "half" in raw.lower()
        return {
    ...
    if re.fullmatch(r"\d{4}", raw):
        year = int(raw)
        return {"year_label": f"Mar {year}", "fiscal_year": year, "quarter": "Q4", "is_ttm": False, "is_half_year": False, "sort_order": year * 10 + 4}

    return {"year_label": raw, "fiscal_year": None, "quarter": None, "is_ttm": False, "is_half_year": False, "sort_order": None}
...
def build_year_dimension(frames: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    year_df = year_df.sort_values(["sort_order", "year_label"], na_position="last").reset_index(drop=True)
    year_df["year_id"] = np.arange(1, len(year_df) + 1)
    return year_df[["year_id", "year_label", "fiscal_year", "quarter", "is_ttm", "is_half_year", "sort_order"]]
...
def normalize_companies(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    keep = [
        ...
        "nse_url", "bse_url", "face_value", "book_value", "about_company", "roce", "roe",
    ]
    return df[[col for col in keep if col in df.columns]].drop_duplicates("symbol"), mapping_df


def normalize_profit_loss(df: pd.DataFrame, dim_year: pd.DataFrame, total_assets_map: pd.DataFrame | None) -> pd.DataFrame:
    ...
    df["interest_coverage"] = np.where(df["interest"] > 0, df["operating_profit"] / df["interest"], np.nan)
    df["asset_turnover"] = np.where(df["total_assets"] > 0, df["sales"] / df["total_assets"], np.nan)
    df["return_on_assets"] = np.where(df["total_assets"] > 0, df["net_profit"] / df["total_assets"] * 100, np.nan)
    keep = [
        "symbol", "year_id", "year_label", "sort_order", "sales", "expenses", "operating_profit",
        "opm_pct", "other_income", "interest", "depreciation", "profit_before_tax", "tax_pct",
        "net_profit", "eps", "dividend_payout_pct", "net_profit_margin_pct", "expense_ratio_pct",
        "interest_coverage", "asset_turnover", "return_on_assets",
    ]
    out = df[[col for col in keep if col in df.columns]]
    return out.drop_duplicates(subset=["symbol", "year_id"], keep="first")


def normalize_balance_sheet(df: pd.DataFrame, dim_year: pd.DataFrame, companies: pd.DataFrame) -> pd.DataFrame:
    df = rename_first_match(df, "symbol", ["company_id", "symbol", "ticker"])
    ...
    shares_outstanding = np.where(df["face_value"] > 0, df["equity_capital"] / df["face_value"], np.nan)
    ...
    df["book_value_per_share"] = np.where(shares_outstanding > 0, net_worth / shares_outstanding, np.nan)
    keep = [
        "symbol", "year_id", "year_label", "sort_order", "equity_capital", "reserves", "borrowings",
        "other_liabilities", "total_liabilities", "fixed_assets", "cwip", "investments", "other_assets",
        "total_assets", "debt_to_equity", "equity_ratio", "book_value_per_share",
    ]
    out = df[[col for col in keep if col in df.columns]]
    return out.drop_duplicates(subset=["symbol", "year_id"], keep="first")


def normalize_cash_flow(df: pd.DataFrame, dim_year: pd.DataFrame, net_profit_map: pd.DataFrame | None) -> pd.DataFrame:
    df = rename_first_match(df, "symbol", ["company_id", "symbol", "ticker"])
    ...
    df["cash_conversion_ratio"] = np.where(df["net_profit"] != 0, df["operating_activity"] / df["net_profit"], np.nan)
    keep = [
        "symbol", "year_id", "year_label", "sort_order", "operating_activity", "investing_activity",
...
def normalize_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for source, target in metric_aliases.items():
        if source in df.columns:
            df[target] = df[source].astype(str).str.extract(r"([-+]?\d+(?:\.\d+)?)").iloc[:, 0].astype(float)
    ...
    keep = ["symbol", "period_label", "compounded_sales_growth_pct", "compounded_profit_growth_pct", "stock_price_cagr_pct", "roe_pct"]
    return df[[col for col in keep if col in df.columns]].dropna(subset=["symbol", "period_label"]).drop_duplicates()
...
def normalize_documents(df: pd.DataFrame) -> pd.DataFrame:
    df = rename_first_match(df, "symbol", ["company_id", "symbol", "ticker"])
    df = rename_first_match(df, "document_url", ["document_url", "annual_report_url", "annual_report", "url"])
    df["symbol"] = df["symbol"].apply(normalize_symbol_value)
    ...
    out = df[[col for col in ["symbol", "year_label", "document_url"] if col in df.columns]].dropna(subset=["symbol", "document_url"])
...
def build_dim_sector(mapping_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sectors["sector_code"] = sectors["sector"].str.upper().str.replace(r"[^A-Z0-9]+", "_", regex=True).str.strip("_")
...
def run(raw_dir: Path, clean_dir: Path) -> None:
    ...
    raw_frames = {name: read_raw_csv(raw_dir / filename) for name, filename in RAW_FILES.items() if (raw_dir / filename).exists()}

    if "companies" not in raw_frames:
        raise FileNotFoundError("companies.csv is required in data/raw. Run 01_extract_from_mysql.py first.")

    year_sources = [raw_frames[name] for name in ["balancesheet", "profitandloss", "cashflow"] if name in raw_frames]
    ...
    for name, frame in raw_frames.items():
        symbol_col = "company_id" if "company_id" in frame.columns else "id" if name == "companies" and "id" in frame.columns else None
        if symbol_col:
    ...
    if missing_symbols:
        ...
        sector_mapping = pd.concat([sector_mapping, pd.DataFrame(stub_mapping)], ignore_index=True).drop_duplicates("symbol")
    companies = companies.drop_duplicates("symbol").sort_values("symbol").reset_index(drop=True)
    sector_mapping = sector_mapping.drop_duplicates("symbol").sort_values("symbol").reset_index(drop=True)
    dim_sector = build_dim_sector(sector_mapping)

    balance_sheet = normalize_balance_sheet(raw_frames.get("balancesheet", pd.DataFrame()), dim_year, companies) if "balancesheet" in raw_frames else pd.DataFrame()
    total_assets_map = balance_sheet[["symbol", "year_id", "total_assets"]] if not balance_sheet.empty else None
    profit_loss = normalize_profit_loss(raw_frames.get("profitandloss", pd.DataFrame()), dim_year, total_assets_map) if "profitandloss" in raw_frames else pd.DataFrame()
    net_profit_map = profit_loss[["symbol", "year_id", "net_profit"]] if not profit_loss.empty else None
    cash_flow = normalize_cash_flow(raw_frames.get("cashflow", pd.DataFrame()), dim_year, net_profit_map) if "cashflow" in raw_frames else pd.DataFrame()
    analysis = normalize_analysis(raw_frames.get("analysis", pd.DataFrame())) if "analysis" in raw_frames else pd.DataFrame()
    pros_cons = normalize_pros_cons(raw_frames.get("prosandcons", pd.DataFrame())) if "prosandcons" in raw_frames else pd.DataFrame()
    documents = normalize_documents(raw_frames.get("documents", pd.DataFrame())) if "documents" in raw_frames else pd.DataFrame()
